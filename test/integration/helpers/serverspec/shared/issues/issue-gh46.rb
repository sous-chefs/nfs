  ...
  context "Issue #46" do
    ...
    context "Unrelated shares are not stairstepping anonuid/anongid" do
      describe command("egrep -v '/tmp/share[0-9] 127.0.0.1\\(rw,sync,root_squash,(anonuid=[0-9]+,anongid=[0-9]+){2,}\\)' /etc/exports") do
        ...
        its(:stdout) { should_not match(/^\/tmp\/share2[.]*anonuid=1001/) }
        its(:stdout) { should_not match(/^\/tmp\/share2[.]*anongid=1001/) }
        its(:stdout) { should_not match(/^\/tmp\/share3[.]*anonuid=1002/) }
        its(:stdout) { should_not match(/^\/tmp\/share3[.]*anongid=1002/) }
      end
    end
  end
end
