shared_examples 'issues::gh46' do
  context 'Issue #46' do
    context 'Uniform anonuid/anongid on unrelated shares' do
      describe command("grep -Ec '\"?/tmp/share[0-9]\"? 127.0.0.1\\([^)]*" \
          "anon[gu]id=[0-9]+,anon[gu]id=[0-9]+[^)]*\\)' /etc/exports") do
    ...
    context 'Unrelated shares are not stairstepping anonuid/anongid' do
      describe command("grep -Ev '\"?/tmp/share[0-9]\"? 127.0.0.1\\([^)]*(" \
          "anon[gu]id=[0-9]+,anon[gu]id=[0-9]+){2,}[^)]*\\)' /etc/exports") do
        its(:exit_status) { should eq 0 }
